...
void imuSensorThread() {
  while(true) {
    // Collect IMU data
    sensors.collectIMUData();
    threads.yield();
  }
}
...
void setup()
{
  ...
  sensors.enableIMUReports(interval);
  
  // Start the sensor thread threads
  threads.addThread(altimeterSensorThread);
  threads.addThread(temperatureSensorThread);
  threads.addThread(imuSensorThread);
}
